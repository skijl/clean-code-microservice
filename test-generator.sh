...
STATIC_FILES_TEST_DIR="/d/scripts/static_files/test"
...
create_service_tests() {
    ...
    service_test_file="$TEST_BASE_DIR/service/${model_name}ServiceImplTest.java"
    service_file="$BASE_DIR/service/impl/${model_name}ServiceImpl.java"
    ...
    echo "import ${base_package_name}.service.impl.${model_name}ServiceImpl;" >> "$service_test_file"
    ...
    if [ -f "temp" ]; then
        ...
        while IFS= read -r service; do
            ...
            echo "    public void testCreate_EntityNotFoundException_${object}NotFound() {" >> "$service_test_file"
            object="${service%Service}"
            service_name=$(echo "${service:0:1}" | tr '[:upper:]' '[:lower:]')${service:1}
            if [ -n "$assertion" ]; then
            ...
            echo "" >> "$service_test_file"
            echo "        EntityNotFoundException exception = assertThrows(EntityNotFoundException.class, () -> {" >> "$service_test_file"
            echo "            ${lowercase_model_name}Service.create(${lowercase_model_name});" >> "$service_test_file"
            echo "        });" >> "$service_test_file"
            echo "" >> "$service_test_file"
    ...
    echo "    @Test" >> "$service_test_file"
    echo "    public void testCreate_DataAccessException() {" >> "$service_test_file"
    process_temp_file "$service_test_file"
    ...
    if [ -f "temp" ]; then
        ...
        while IFS= read -r service; do
            ...
            lowercase_object=$(echo "${object:0:1}" | tr '[:upper:]' '[:lower:]')${object:1}
            echo "    @Test" >> "$service_test_file"
            echo "    public void testUpdate_EntityNotFoundException_${object}NotFound() {" >> "$service_test_file"
            ...
            echo "" >> "$service_test_file"
            echo "        EntityNotFoundException exception = assertThrows(EntityNotFoundException.class, () -> {" >> "$service_test_file"
            echo "            ${lowercase_model_name}Service.update(Static${model_name}.ID, ${lowercase_model_name});" >> "$service_test_file"
            echo "        });" >> "$service_test_file"
            echo "" >> "$service_test_file"
            echo "        assertNotNull(exception);" >> "$service_test_file"
            echo "        assertEquals(\"${object} not found\", exception.getMessage());" >> "$service_test_file"
...
create_controller_tests() {
    ...
    sed "s~\${model_name}~$model_name~g; s~\${lowercase_model_name}~$lowercase_model_name~g; s~\${controller_api}~$controller_api~g; s~\${class_name}~$class_name~g" "$STATIC_FILES_TEST_DIR/controller/static1" >> "$controller_test_file"
